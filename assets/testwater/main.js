...
window.onload = function() {
  ...
  function onresize() {
    ...
    gl.viewport(-gl.canvas.width/10, -gl.canvas.height/1.43, gl.canvas.width*1.2, gl.canvas.height*1.2);
  ...
  //鼠标拖拽操作。接受两个参数x和y，代表鼠标点击位置的横坐标和纵坐标。
  ...
  //  处理鼠标拖拽时候的交互操作的，该函数接收鼠标的x和y坐标作为参数。
  // MODE_ADD_DROPS：在水面上添加一些波纹，例如在某个点上添加一块石头等。
  // MODE_MOVE_SPHERE：移动球形物体，例如在鼠标移动过程中拖动球体。
  // MODE_ORBIT_CAMERA：用于摄像机转动。
